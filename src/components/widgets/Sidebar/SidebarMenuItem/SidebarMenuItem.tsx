...
const SidebarMenuItem: FunctionComponent<Props> = ({ item }): JSX.Element => {
  ...
  const notActiveItemClasses =
    'text-gray-700 dark:text-gray-400 font-medium transition hover:border-gray-200 dark:hover:border-zinc-900'
  const activeLineClasses =
    'before:absolute before:w-[5px] before:h-full before:bg-blue-400 dark:before:bg-white before:left-[-10px] before:rounded-r-full before:transition before:opacity-0'
  const activeOpenItem =
    'text-blue-400 bg-blue-50 font-semibold dark:text-white dark:bg-zinc-900'
